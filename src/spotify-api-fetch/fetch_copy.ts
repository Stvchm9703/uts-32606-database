...
export const main = async (arg: any, env: any) => {
  ...
  const fet_in_file_album = await Deno.readTextFile("./data/album-list.txt");


  let isDryFetch = !!arg['dry-fetch'];
  let fetched_artist = [];
  let fetched_album = [];
  let fetched_album_id = [...(new Set(fet_in_file_album.split('\n')))];
  let fetched_track = [];
  ...
  const _bluk_fetch_artist = async () => {
    ...
    // if (Deno.)
    await Deno.writeTextFile("./data/artist.json", JSON.stringify(fetched_artist), { append: false, create: true, createNew: false })
  ...
  const _bluk_fetch_album = async () => {
    ...
    fetched_album_id = [...(new Set([...fetched_album_id, ...fetched_album.map(elm => elm.id)]))]
    console.log(fetched_album_id.length);
    Deno.writeTextFileSync(`./data/album-list.txt`, fetched_album_id.join(`\n`), { append: false, create: true, createNew: false })
  }

  const _bluk_fetch_track = async () => {
    for (const elm of fetched_album) {
      try {
        const isExist = await exists(`./data/track-${elm.id}.json`);
        console.log(isExist)
        if (!isExist) {
          await Timeout.wait(500);
